...
if ($team) {
    try {
        $m = $pdo->prepare("SELECT id,username,role FROM Player WHERE team_id=? AND deleted_at IS NULL ORDER BY (id=?) DESC, username");
    ...
    try {
        $t = $pdo->prepare("
            ...
            WHERE tt.team_id=? ORDER BY t.start_date DESC LIMIT 5
        ");
        $t->execute([$team['id']]);
    ...
    try {
        $st = $pdo->prepare("
            SELECT COUNT(*) AS matches,
                   COALESCE(SUM((home_team_id=:t AND score_team1>score_team2)
                                OR(away_team_id=:t AND score_team2>score_team1)),0) AS wins
            FROM   Matches
            WHERE  (home_team_id=:t OR away_team_id=:t) AND score_team1 IS NOT NULL AND deleted_at IS NULL
        ");
...
try {
    $games = $pdo->query("SELECT name FROM Game WHERE deleted_at IS NULL ORDER BY name")->fetchAll(PDO::FETCH_COLUMN);
...
function regionSelectorHtml(string $id, string $hiddenId, string $initial = ''): string {
    ...
    <div class="tm-region-wrap" id="{$id}">
        ...
        <input type="hidden" name="region" id="{$hiddenId}" value="{$initial}">
    </div>
    HTML;
}
?>
                    ...
                    <form id="create-team-form" enctype="multipart/form-data" novalidate>
                        ...
                        <div class="tm-form-grid">
                            <div class="tm-field">
                                ...
                                <input class="tm-input" type="text" name="name" placeholder="NightFall" maxlength="50" required>
                            </div>
                            <div class="tm-field">
                                <label class="tm-label">Tag * (2–6 chars)</label>
                                <input class="tm-input" type="text" name="tag" placeholder="NF" maxlength="6" minlength="2" required>
                            </div>
                            <div class="tm-field">
                        ...
                        <div class="tm-form-actions">
                            <button type="button" class="tm-btn-ghost" onclick="TeamApp.togglePanel('createPanel')">Cancel</button>
...
<div class="tm-header">
    ...
    <div class="tm-info">
        ...
        <div class="tm-code-row">
            ...
            <span id="tm-invite-code" class="tm-code-value"><?= htmlspecialchars($team['invitation_code']) ?></span>
            <button class="tm-code-copy" onclick="TeamApp.copyCode()" title="Copy code">
...
<div class="tm-stats">
    ...
    <div class="tm-stat">
        ...
        <div class="tm-wr-bar-mini">
            <div class="tm-wr-fill-mini <?= $wr>=60?'high':($wr>=40?'mid':'low') ?>"
                 style="width:<?= $wr ?>%"></div>
        </div>
        <?php endif; ?>
    </div>
        ...
        <div class="tm-panel">
            ...
            <form id="edit-team-form" enctype="multipart/form-data" novalidate>
                ...
                <div class="tm-field" style="margin-bottom:14px;">
                    ...
                    <label class="tm-avatar-upload-area" for="edit-avatar-input">
                        <div class="tm-avatar-preview" id="edit-avatar-preview">
                            <?php if (!empty($team['logo_url'])): ?>
                                <img src="<?= htmlspecialchars($team['logo_url']) ?>" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:inherit;">
                            <?php else: ?>
                    ...
                    <?php if (!empty($team['logo_url'])): ?>
                    <button type="button" class="tm-remove-avatar-btn" onclick="TeamApp.removeAvatar()">
                        ✕ Remove current logo
                    </button>
                    <?php endif; ?>
                </div>
                ...
                <div class="tm-form-grid">
                    <div class="tm-field">
                        <label class="tm-label">Team Name *</label>
                        <input class="tm-input" type="text" name="name" value="<?= htmlspecialchars($team['name']) ?>" maxlength="50" required>
                    </div>
                    <div class="tm-field">
                        <label class="tm-label">Tag * (2–6)</label>
                        <input class="tm-input" type="text" name="tag" value="<?= htmlspecialchars($team['tag'] ?? '') ?>" maxlength="6" required>
                    </div>
                    <div class="tm-field">
                        <label class="tm-label">Game</label>
                ...
                <div class="tm-form-actions">
                    <button type="button" class="tm-btn-ghost" onclick="TeamApp.closePanel('editPanel')">Cancel</button>
...
<!-- INVITE PANEL (placeholder — full implementation in Part 2) ─── -->
        ...
        <div class="tm-panel tm-panel--accent">
            ...
            <form id="invite-form" novalidate>
                <div style="display:flex; gap:8px;">
                    <input class="tm-input" type="text" name="invite_username" id="invite-username-input"
                           placeholder="Enter username..." style="flex:1;" required>
                    <button type="submit" class="tm-btn-primary">Send Invite</button>
                </div>
                <div id="invite-feedback" style="font-size:12px;margin-top:6px;color:var(--text-muted);min-height:18px;"></div>
            </form>
            <div class="tm-panel-note">Members: <?= count($members) ?>/6 · They'll receive an email invitation (Part 2)</div>
        </div>
    </div>
</div>
<?php endif; ?>
...
<div class="tm-two-col">
    ...
    <div class="tm-card">
        <div class="tm-card-head">
            ...
            <span id="tm-member-count" class="op-td-muted" style="font-size:12px;"><?= count($members) ?>/6</span>
        </div>
    ...
    <div class="tm-card">
        ...
        <?php else: ?>
            <?php foreach ($tournaments as $t):
                $sm = [
                    'live'         => ['tm-dot--live',     'Live'],
                    'registration' => ['tm-dot--upcoming', 'Open'],
                    'upcoming'     => ['tm-dot--upcoming', 'Upcoming'],
                    'finished'     => ['tm-dot--done',     'Finished'],
                ];
                [$dotCls, $stLabel] = $sm[$t['status']] ?? ['tm-dot--done', ucfirst($t['status'])];
            ?>
            <div class="tm-tournament-row">
                ...
                <span class="tm-result <?= $t['status'] === 'live' ? 'tm-result--ongoing' : ($t['status'] !== 'finished' ? 'tm-result--soon' : '') ?>">
                    <?= $stLabel ?>
                </span>
            </div>
            <?php endforeach; ?>
